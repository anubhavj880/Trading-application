 ...
 *
 */
public class LiveCoinSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(LiveCoinSingletonClass.class);
	private static LiveCoinSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = LiveCoinSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private LiveCoinSingletonClass() {

	}

	public static LiveCoinSingletonClass getInstance() throws IOException {
		if (adlConnectionObj == null) {
			adlConnectionObj = new LiveCoinSingletonClass();
		}
