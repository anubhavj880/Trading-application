package com.biz.coinbase2datalake.util;
 ...
 *
 */
public class CoinBaseSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(CoinBaseSingletonClass.class);
	private static CoinBaseSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = CoinBaseSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private CoinBaseSingletonClass() {

	}

	public static CoinBaseSingletonClass getInstance() throws IOException {
		if (adlConnectionObj == null) {
			adlConnectionObj = new CoinBaseSingletonClass();
		}
