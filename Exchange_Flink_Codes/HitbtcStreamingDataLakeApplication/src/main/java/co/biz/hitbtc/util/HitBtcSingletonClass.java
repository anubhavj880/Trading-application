 ...
 *
 */
public class HitBtcSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(HitBtcSingletonClass.class);
	private static HitBtcSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = HitBtcSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private HitBtcSingletonClass() {

	}

	public static HitBtcSingletonClass getInstance() throws IOException {
		if (adlConnectionObj == null) {
			adlConnectionObj = new HitBtcSingletonClass();
		}
