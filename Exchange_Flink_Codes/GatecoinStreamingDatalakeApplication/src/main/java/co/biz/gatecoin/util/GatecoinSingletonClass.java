 ...
 *
 */
public class GatecoinSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(GatecoinSingletonClass.class);
	private static GatecoinSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = GatecoinSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private GatecoinSingletonClass() {

	}

	public static GatecoinSingletonClass getInstance() throws IOException {
		if (adlConnectionObj == null) {
			adlConnectionObj = new GatecoinSingletonClass();
		}
