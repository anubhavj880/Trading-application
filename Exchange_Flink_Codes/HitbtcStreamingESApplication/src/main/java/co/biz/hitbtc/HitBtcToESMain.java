...
import co.biz.hitbtc.sink.ES_Sink;
...
public class HitBtcToESMain {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(HitBtcToESMain.class);
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			ethbtcFileStream = new FileInputStream("/home/bizruntime/state/hitbtcESethbtc.txt");
    	...
    	try {
    		kafkaPropStream = HitBtcToESMain.class.getClassLoader().getResourceAsStream("kafka.properties");
    		esPropStream = HitBtcToESMain.class.getClassLoader().getResourceAsStream("es.properties");
...
////		xmrbtc_TickerStream.addSink(ES_Sink.getESTickerSink()).setParallelism(3).name("XMRBTC_Ticker_ESSink");
		...
		streamEnv.execute("HitBtc_Streaming_ES_App");
