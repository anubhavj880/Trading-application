 ...
 *
 */
public class KrakenSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(KrakenSingletonClass.class);
	private static KrakenSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = KrakenSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private KrakenSingletonClass() {

	}

	public static KrakenSingletonClass getInstance() throws IOException {
		if (adlConnectionObj == null) {
			adlConnectionObj = new KrakenSingletonClass();
		}
