package com.biz.bitfinex2datalake.util;
...
import com.biz.bitfinex2datalake.BitfinexToDataLakeMain;
 ...
 *
 */
public class BitfinexSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(BitfinexSingletonClass.class);
	private static BitfinexSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = BitfinexSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private BitfinexSingletonClass() {

	}

	public static  BitfinexSingletonClass getInstance() throws IOException {
		
			if (adlConnectionObj == null) {
				adlConnectionObj = new BitfinexSingletonClass();
			}
