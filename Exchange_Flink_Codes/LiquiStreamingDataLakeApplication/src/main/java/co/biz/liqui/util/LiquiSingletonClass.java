 ...
 *
 */
public class LiquiSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(LiquiSingletonClass.class);
	private static LiquiSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = LiquiSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private LiquiSingletonClass() {

	}

	public static LiquiSingletonClass getInstance() throws IOException {
		if (adlConnectionObj == null) {
			adlConnectionObj = new LiquiSingletonClass();
		}
