 ...
 *
 */
public class TheRockSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(TheRockSingletonClass.class);
	private static TheRockSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = TheRockSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private TheRockSingletonClass() {

	}

	public static  TheRockSingletonClass getInstance() throws IOException {
		if (adlConnectionObj == null) {
			adlConnectionObj = new TheRockSingletonClass();
		}
