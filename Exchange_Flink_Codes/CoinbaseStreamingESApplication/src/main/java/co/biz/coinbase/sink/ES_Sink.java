...
import co.biz.coinbase.exceptions.ElasticSearchDataException;
	...
	public static ElasticsearchSinkFunction<String> getESSinkFunction(String indexname, String typename) {
		...
		ElasticsearchSinkFunction<String> obj = new ElasticsearchSinkFunction<String>() {
			...
			public IndexRequest createIndexRequest(String element) throws ElasticSearchDataException {
				...
				if (indexname.equals("coinbasebestbidask") && (json.length() > 0) && (json.has("TimeStamp") && (json.getString("TimeStamp") != null)) && (json.has("BestBid")&& json.getJSONObject("BestBid").length() > 0) && (json.has("BestAsk")&& json.getJSONObject("BestAsk").length() > 0)) {
					indexRequest = Requests.indexRequest().index(indexname).type(typename).source(element);
					return indexRequest;
				}
				else if(indexname.equals("coinbasebestbidask") && (json.length() == 0))
				{
				...
				else if(indexname.equals("coinbasebestbidask") && (json.length() > 0) && (json.has("TimeStamp") && (json.getString("TimeStamp") == null || json.getString("TimeStamp").isEmpty())))
				{
					throw new ElasticSearchDataException("The bestbidask TimeStampdata to elasticsearch index is empty or null");
				}
				else if(indexname.equals("coinbasebestbidask") && (json.length() > 0) && (json.has("BestBid") && (json.getJSONObject("BestBid") == null || json.getJSONObject("BestBid").length() == 0)))
				{
					throw new ElasticSearchDataException("The bestbidask BestBiddata to elasticsearch index is empty or null");
				}
				else if(indexname.equals("coinbasebestbidask") && (json.length() > 0) && (json.has("BestAsk") && (json.getJSONObject("BestAsk") == null || json.getJSONObject("BestAsk").length() == 0)))
				{
					throw new ElasticSearchDataException("The bestbidask BestBiddata to elasticsearch index is empty or null");
				}
