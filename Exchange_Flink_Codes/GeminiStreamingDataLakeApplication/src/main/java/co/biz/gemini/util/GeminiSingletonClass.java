 ...
 *
 */
public class GeminiSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(GeminiSingletonClass.class);
	private static GeminiSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = GeminiSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private GeminiSingletonClass() {

	}

	public static  GeminiSingletonClass getInstance() throws IOException {
		
			if (adlConnectionObj == null) {
				adlConnectionObj = new GeminiSingletonClass();
			}
