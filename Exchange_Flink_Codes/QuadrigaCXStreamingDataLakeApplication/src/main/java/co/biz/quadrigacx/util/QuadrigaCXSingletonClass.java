package co.biz.quadrigacx.util;
 ...
 *
 */
public class QuadrigaCXSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(QuadrigaCXSingletonClass.class);
	private static QuadrigaCXSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = QuadrigaCXSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private QuadrigaCXSingletonClass() {

	}

	public static QuadrigaCXSingletonClass getInstance() throws IOException {
		if (adlConnectionObj == null) {
			adlConnectionObj = new QuadrigaCXSingletonClass();
		}
