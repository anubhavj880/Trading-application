...
import co.biz.poloniex.sink.PoloniexEthbtcADLSink;
import co.biz.poloniex.util.BestBidAskCalculator;
import co.biz.poloniex.util.MidPointCalculator;


public class PoloniexToADLMain {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(PoloniexToADLMain.class);
	...
	public static void main(String[] args) throws Exception {
    	...
    	try {
    		kafkaPropStream = PoloniexToADLMain.class.getClassLoader().getResourceAsStream("kafka.properties");
		...
		ethbtc_RawOrderStream.addSink(new PoloniexEthbtcADLSink<String>()).name("ETHBTC_RawOrder_ADLSink").setParallelism(1);
		...
		ethbtc_MidPointStream.addSink(new PoloniexEthbtcADLSink<String>()).setParallelism(1).name("ETHBTC_Midpoint_ADLSink");
		ethbtc_BestBidAskOrderStream.addSink(new PoloniexEthbtcADLSink<String>()).name("ETHBTC_BestBidAskOrder_ADLSink").setParallelism(1);
		/**
		 * Adding the Poloniex-ETHBTC-Trade source to the execution environment
		 */
		DataStream<String> ethbtc_TradeStream = env.addSource(
				new FlinkKafkaConsumer010<String>("Poloniex-BTC_ETH-Trade", new SimpleStringSchema(), kafkaProp),
				"Kafka_Poloniex_ETHBTC_Trade_Source").setParallelism(2);
		ethbtc_TradeStream.addSink(new PoloniexEthbtcADLSink<String>()).setParallelism(1).name("ETHBTC_Trade_ADLSink");


		/**
		 * Adding the Poloniex-ETHBTC-Ticker source to the execution environment
		 */
		DataStream<String> ethbtc_TickerStream = env.addSource(
				new FlinkKafkaConsumer010<String>("Poloniex-BTC_ETH-Ticker", new SimpleStringSchema(), kafkaProp),
				"Kafka_Poloniex_ETHBTC_Ticker_Source").setParallelism(2);
		ethbtc_TickerStream.addSink(new PoloniexEthbtcADLSink<String>()).setParallelism(1).name("ETHBTC_Ticker_ADLSink");
...
////		xrpbtc_TickerStream.addSink(ES_Sink.getESTickerSink()).setParallelism(3).name("XRPBTC_Ticker_ESSink");
		...
		env.execute("Poloniex_Streaming_ADL_App");
