 ...
 *
 */
public class YobitSingletonClass {
	private final static Logger slf4jLogger = LoggerFactory.getLogger(YobitSingletonClass.class);
	private static YobitSingletonClass adlConnectionObj;
	private static Properties adlProp = new Properties();
	static{
		
    	InputStream adlPropStream = null;
    	try {
    		adlPropStream = YobitSingletonClass.class.getClassLoader().getResourceAsStream("datalake.properties");
	...
	private YobitSingletonClass() {

	}

	public static YobitSingletonClass getInstance() throws IOException {
		if (adlConnectionObj == null) {
			adlConnectionObj = new YobitSingletonClass();
		}
