...
namespace Stateless1
{
    internal static class Program
    {
                ...
                foreach (var operation in inputValidator.GetKeyOperations())
                {
                    try
                    {
                        switch (operation)
                        {
                            ...
                            case KeyOperationType.ENCRYPT_ApiKey:
                                Encrypt_ApiKey(keyBundle);
                                break;

                            case KeyOperationType.ENCRYPT_Secret:
                                EncryptSecret(keyBundle);
                                break;
        ...
        private static SecretBundle CreateApiKey(out string secretName, string value)
        {
           secretName = inputValidator.GetApiName();
        ...
        private static SecretBundle CreateSecret(out string secretName, string value)
        {
        ...
        private static void Encrypt_ApiKey(KeyBundle key)
        {
            KeyOperationResult operationResult;
            ...
            CreateApiKey(out secretName, Convert.ToBase64String(operationResult.Result));
        }
        private static void EncryptSecret(KeyBundle key)
        {
            KeyOperationResult operationResult;

            var algorithm = inputValidator.GetEncryptionAlgorithm();
            var plainText = inputValidator.GetPlainTextSecret();
            ...
            CreateSecret(out secretName, Convert.ToBase64String(operationResult.Result));
        }
        ...
        private static void Decrypt(KeyBundle key)
        {
            ...
            ServiceEventSource.Current.Message("===========> Response  for stock  Current Balance: {0}", response.Content);
