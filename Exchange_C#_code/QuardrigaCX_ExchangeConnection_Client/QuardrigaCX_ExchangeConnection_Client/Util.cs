...
namespace QuardrigaCX_ExchangeConnection_Client
{
    class Util
    {
        ...
        private static QuadrigaCX quadrigaCXObj = new QuadrigaCX();
        private String uri = "";
        private String topicName = "";
        private long quadrigaCXTradeId = 0;
      ...
      /* public async Task SendHttpRequest(String uri, String topicName)
                ...
                using (var client = new HttpClient())
                {
                    try
                    {
                        ...
                        if ((response != null) && response.IsSuccessStatusCode)
                        {
                            ...
                            if (topicName.Contains("Trade"))
                            {
                                var tradeArray = JsonConvert.DeserializeObject<JArray>(exchangeData);
                                String[] topicSplit = topicName.Split('-');

                                if (quadrigaCXTradeId == 0)
                                {
                                    quadrigaCXTradeId = (long)tradeArray[0]["tid"];
                                ...
                                else
                                {
                                    foreach (var tradeJson in tradeArray)
                                    {
                                        if (quadrigaCXTradeId < (long)(tradeJson.ToObject<JObject>()["tid"]))
                                        {
                                            await SendToKafka(tradeJson.ToString(), topicName);
                                        }
                                    }
                                    quadrigaCXTradeId = tradeArray[0]["tid"].ToObject<long>();
                                }
                            }
        ...
        private static async Task SendToKafka(String stringData, String topic)
        {
            ...
            {
                await quadrigaCXObj.createOrderBookJson(stringData, topic);
            }
            else if (topicSplit[2].ToLower().Equals("trade"))
            {
                await quadrigaCXObj.createTradeBookJson(stringData, topic);
            }
            else if (topicSplit[2].ToLower().Equals("ticker"))
            {
                await quadrigaCXObj.createTickerJson(stringData, topic);
            }
        }
    }
}
